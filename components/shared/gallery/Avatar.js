...
const Avatar = ({ avatarUrl, onAvatarClick }) => {
  const imageNumber = avatarUrl.split('/').pop().split('.')[0]; 
  return (
    <div className="g-avatar ml-2">
      <div className="w-full">
        <div
          className="bg-sub-black rounded-md shadow-xl duration-300 hover:scale-105 dark:hover:bg-gray-700 hover:bg-gray-200 sm:p-3 p-2"
          onClick={onAvatarClick}
        >
          <div className="w-full sm:mb-3 mb-2">
            <img className="w-full shadow-md rounded-full" src={avatarUrl} alt="circle avatar image" />
          </div>
