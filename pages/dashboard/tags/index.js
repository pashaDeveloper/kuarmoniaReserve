  ...
  const ListTag = () => {
    ...
    const itemsPerPage = 7; // تعداد آیتم در هر صفحه
      ...
      <>
        ...
        <div className="mt-6 md:flex md:flex-row-reverse md:items-center md:justify-between ">
          <div className="inline-flex  overflow-hidden bg-white border divide-x rounded-lg dark:bg-blue-500 rtl:flex-row dark:border-whitedark:divide-white">
            <button className="px-5 py-2 text-xs font-medium text-gray-600 transition-colors duration-200 bg-gray-100 sm:text-sm dark:bg-blue-600 dark:text-gray-300">
              همه 
            </button>
            <button className="px-5 py-2 text-xs font-medium text-gray-600 transition-colors duration-200 sm:text-sm dark:hover:bg-blue-600 dark:text-gray-300 hover:bg-gray-100">
              فعال
            </button>
            <button className="px-5 py-2 text-xs font-medium text-gray-600 transition-colors duration-200 sm:text-sm dark:hover:bg-blue-600  dark:text-gray-300 hover:bg-gray-100">
              غیر فعال
          ...
          <div className="mt-8 w-full grid grid-cols-12 text-slate-400 ">
  <div className="col-span-11 lg:col-span-2  text-sm">عنوان</div>
  <div className="lg:col-span-4 lg:flex hidden text-sm md:block">توضیحات</div>
  <div className="lg:col-span-2 hidden lg:flex col-span-2 text-sm text-right">ربات</div>
  <div className="lg:col-span-3 lg:flex hidden justify-right text-right items-center gap-x-1 gap-y-1 flex-wrap  text-sm">کلمات کلیدی</div>
  <div className="col-span-1 md:block">عملیات</div>
        ...
        data.data.map((tag) => (
                <div
                  key={tag._id}
                  className="mt-4 p-1 grid grid-cols-12 rounded-xl cursor-pointer border border-gray-200 gap-2 dark:border-white/10 dark:bg-slate-800 bg-white transition-all dark:hover:border-slate-700 hover:border-slate-100 hover:bg-green-100 dark:hover:bg-gray-800 dark:text-slate-100 px-4"
                >
                  <div className="col-span-11 lg:col-span-2 text-center flex items-center">
                    ...
                    <article className="flex-col flex gap-y-2  ">
                      <span className="line-clamp-1 text-base ">
                        <span className=" flex">{tag.title}</span>
                      </span>
                      <span className="line-clamp-1 text-base ">
  ...
  <div className="lg:col-span-2 hidden lg:flex col-span-2 justify-right text-center items-center gap-x-2">
    {tag.robots && tag.robots.length > 0 ? (
      tag.robots.map((robot, index) => (
        <span key={index} className="line-clamp-1 flex gap-x-1 cursor-pointer rounded-lg border border-green-700/5 dark:border-blue-500/5 bg-green-800/5 dark:bg-blue-500/5 px-2 py-1 text-green-500 dark:text-blue-500  transition-colors hover:border-green-700/10 dark:hover:border-blue-500/10 hover:bg-green-700/10 dark:hover:bg-blue-500/10 hover:!opacity-100  group-hover:opacity-70 text-sm">
